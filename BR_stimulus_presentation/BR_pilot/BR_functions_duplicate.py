...
def runRivalryTrials(thisExp,
    ...
    bottomStim = visual.ImageStim(windowObject, mask='raisedCos', image=bottomImage, size=(gratingSize, gratingSize), opacity=1.0)
    # previously mask='circle'
    bottomStim.draw()
    topStim = visual.ImageStim(windowObject, mask='raisedCos', image=topImage, size=(gratingSize, gratingSize), opacity=0.5)
    topStim.draw()
    windowObject.flip()
    ...
    while rivalryBuffer.getTime() > 0: # countdown for the rivalry duration
        ...
        if len(keyResp) > 0 :
            thisExp.addData('Flag', 'cut off by trial end')
            allResponses_RT.extend([key.rt for key in keyResp])
            allResponses.extend(keyResp)
            allResponses_Dur.extend([key.duration for key in keyResp])
            allResponses_Names.extend([key.name for key in keyResp])
...
#    if rivalryTime.getTime() == 0.0 and key.duration is None:
    ...
    # save key responses to thisExp
    thisExp.addData('keyDur', allResponses_Dur)
    thisExp.addData('keyRT', allResponses_RT)
    thisExp.addData('keyName', allResponses_Names)
    thisExp.addData('keys', allResponses)
    thisExp.addData('expTime-TrialStart', trialStart)
    thisExp.addData('expTime-TrialEnd', trialEnd)
    thisExp.addData('eventKeys', additionalKeySave)
...
def runGlassesDemo(keyboardObject, 
    ...
    bottomStim = visual.ImageStim(windowObject, mask="raisedCos", image=bottomImage, size=(gratingSize, gratingSize), opacity=1.0, maskParams={'fringeWidth':0.5})
    bottomStim.draw()
    topStim = visual.ImageStim(windowObject, mask="raisedCos", image=topImage, size=(gratingSize, gratingSize), opacity=0.5, maskParams={'fringeWidth':0.5})
    topStim.draw()
    windowObject.flip()
